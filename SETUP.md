...
- **Package-view follow-up reminder** (`/api/packages/view`, `/api/packages/interest`, `/api/packages/followup`) — when a visitor clicks "View Packages," that's logged; `PACKAGE_VIEW_FOLLOWUP_DELAY_MINUTES` (in `lib/config/packageFollowUp.ts`, default 5) later, if they haven't clicked "Connect on WhatsApp" on a package by then, you get a Telegram nudge with their details and a **ready wa.me link, pre-filled, addressed to them** — tap it and it opens WhatsApp with the message already written, ready to send from your real number. Nothing is ever sent to the visitor automatically; a real message to a client who hasn't messaged first requires WhatsApp Business Platform verification with Meta, which this deliberately avoids (see the note below).

  The "wait a few minutes, then check in" part needs a scheduler — serverless functions can't just sleep — via [Upstash QStash](https://upstash.com/docs/qstash/overall/getstarted), a free hosted queue built for exactly this ("call this URL again in N seconds"). To go live:
